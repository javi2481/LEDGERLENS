...
Filesystem `openspec/changes/claimprint-ragflow/tasks.md` has every task checked. Engram `sdd/claimprint-ragflow/tasks` (#116) still has unchecked boxes (planning snapshot); apply updated the OpenSpec file. Completeness is judged from the OpenSpec tasks file.
...
- Engram `sdd/claimprint-ragflow/tasks` is stale (unchecked) relative to OpenSpec `tasks.md` (all `[x]`).
...
- Ignore leftover `openspec/changes/wiped-gradio-mvp/` (design already says so).
