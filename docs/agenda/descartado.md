...
|------|---------------------|
| **Docling como parser default** | Cliente RAGFlow v0.26.4 no manda `page_range` (#17450). Reemplazado por [MinerU pipeline](mineru-pipeline.md). Serve ya no está en el overlay. Graph sigue en agenda. |
...
| **Proxy LiteLLM (sidecar Compose)** | Innecesario para este demo: RAGFlow ya elige Groq / Ollama / Voyage en Model providers. El hop extra no aportaba. OpenRouter Nano `:free` no es el default. El SDK LiteLLM de Docling Graph **no** es este sidecar y sigue en agenda. |

Archivos que había en esta carpeta y se eliminaron: `mineru-docling.md`, `tei-embeddings.md`, `e2e-16gb.md`, `openrouter-larger-models.md`, `litellm.md`.
